 ...
 *
 * <p>
 * 背景    : Heyo Android 端首页需要实现类似 探探 卡片左右滑动的效果，而且，卡片自身是需要支持上下滚动的
 * <p>
 * 技术方案 :
 * 1. RecyclerView 的左右滑动效果官方已经实现，详情查看 {@link SimpleCallback}
 * 2. 由于官方默认会拦截触摸事件{@link mOnItemTouchListener}，因此我们需要重写 OnItemTouchListener ，自行实现事件分发
 * </p>
 *
 * <p>
 * <p>
    ...
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        ...
        public boolean onInterceptTouchEvent(@NonNull RecyclerView recyclerView,
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                offsetX += Math.abs(mLastMoveX - curX);
                offsetY += Math.abs(mLastMoveY - curY);
                mLastMoveX = curX;
                mLastMoveY = curY;
            ...
            // 想要左右滑动卡片需要满足三个条件 ：
            // 1、当前的卡片被选中，官方默认实现了
            // 2、x 轴左右的横向的滑动距离大于某个值
            // 3、y 轴上下纵向的滑动距离小于某个值
            float limitX = recyclerView.getWidth() != 0 ? recyclerView.getWidth() * 0.01f : 10f;
            float limitY = recyclerView.getHeight() != 0 ? recyclerView.getHeight() * 0.01f : 20f;

            if (DEBUG) {
                Log.e(TAG, "offsetX = " + offsetX + " , offsetY = " + offsetY + " , limitX = " + limitX + " , limitY = " + limitY + " , width = " + recyclerView.getWidth() + " , height = " + recyclerView.getHeight());
            }

            return mSelected != null && offsetX > limitX && offsetY < limitY;
        }
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                switch (swipeDir) {
                    ...
                    case END:
                        targetTranslateY = 0;
                        targetTranslateX = Math.signum(mDx) * mRecyclerView.getWidth() * 1.2f;
                        break;
                    case UP:
                    case DOWN:
                        targetTranslateX = 0;
                        targetTranslateY = Math.signum(mDy) * mRecyclerView.getHeight() * 1.2f;
                        break;
                    default:
                        targetTranslateX = 0;
                        targetTranslateY = 0;
                ...
                stringBuilder.append("animationType = ").append(animationType).append(" , ");
                stringBuilder.append("currentTranslateX = ").append(currentTranslateX).append(" , ");
                stringBuilder.append("currentTranslateY = ").append(currentTranslateY).append(" , ");
                stringBuilder.append("targetTranslateX = ").append(targetTranslateX).append(" , ");
                stringBuilder.append("targetTranslateY = ").append(targetTranslateY).append(" , ");
